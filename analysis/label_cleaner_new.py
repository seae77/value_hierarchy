...
try:
    ...
    # We only need the column names, not the data
    df_headers = pd.read_excel(INPUT_EXCEL_FILE, sheet_name=SHEET_NAME, nrows=0)
    column_names = list(df_headers.columns)
    print(f"Successfully read column headers from sheet '{SHEET_NAME if SHEET_NAME else 'Default'}' in {INPUT_EXCEL_FILE}.")

    # Display all column names with their indices
    print("\nAll column headers:")
    for i, col in enumerate(column_names):
        print(f"  {i+1}. {col}")

    # Define label columns as all columns from column number 14 to the end
    label_columns = column_names[13:]  # 0-based indexing, so column 14 is at index 13
    ...
    print("\nColumn headers selected for cleaning (from column 14 onwards):")
    for i, col in enumerate(label_columns):
        print(f"  {i+1}. {col}")

    # Create a dictionary to store the column renames
    column_renames = {}
    total_errors = 0
    ...
    for column in label_columns:
        # Apply the cleaning function to the column name
        cleaned_column = clean_and_refactor_label(column)

        # Check if there was an error in cleaning
        if cleaned_column.startswith("PARSE_ERROR"):
            print(f"  Warning: Could not clean column header '{column}'")
            print(f"    Error: {cleaned_column}")
            total_errors += 1
        else:
            print(f"  Original: '{column}'")
            print(f"  Cleaned:  '{cleaned_column}'")
            column_renames[column] = cleaned_column

    if total_errors > 0:
        print(f"\nTotal parsing errors: {total_errors}")
        print("Column headers with parsing errors will keep their original names.")

    # Now read the full data and apply the column renames
    print("\nReading full data to apply cleaned column headers...")
    df = pd.read_excel(INPUT_EXCEL_FILE, sheet_name=SHEET_NAME)

    # Rename the columns in the dataframe
    df = df.rename(columns=column_renames)

    print("\nColumn header cleaning completed.")

    # Display the cleaned column headers
    print("\nFinal column headers:")
    for i, col in enumerate(df.columns):
        if i >= 13:  # Only show columns from index 13 (column 14) onwards
            print(f"  {i+1}. {col}")

    # Write the modified DataFrame to a new Excel file
    print(f"\nWriting data with cleaned column headers to {OUTPUT_EXCEL_FILE}...")
    df.to_excel(OUTPUT_EXCEL_FILE, sheet_name='Cleaned Data', index=False)
    print(f"Successfully saved data with cleaned column names to {OUTPUT_EXCEL_FILE}")
